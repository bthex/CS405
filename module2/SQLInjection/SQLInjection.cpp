...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  //  something more generic
    ...
    // Use RegEx to check string for
    // the presence of 'word'='word'
    // and digit=digit

    std::regex re("('(.*)[a-zA-Z](.*)'='(.*)[a-zA-Z](.*)')|([0-9]=[0-9])");
    std::smatch match;

    if (std::regex_search(sql.begin(), sql.end(), match, re) && match.size() > 1) {
        std::cout << "SQL Injection attack detected" << std::endl;
        return false;
    }
...
// You can change main by adding stuff to it, but all the existing code must remain, and be
