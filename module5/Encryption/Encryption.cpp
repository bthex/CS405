...
std::string encrypt_decrypt(const std::string& source, const std::string& key)
{
  ...
  { // TODO: student need to change the next line from output[i] = source[i]
    ...
    output[i] = static_cast<char>(source[i] ^ key[i % source_length]);
  }
...
void save_data_file(const std::string& filename, const std::string& student_name, const std::string& key, const std::string& data)
{
  ...
  std::fstream saveFileName;
  // Open save file
  saveFileName.open(filename, std::ios::out);
  // Check if file is open
  if (saveFileName.is_open())
  {
      // Write student name to file
      saveFileName << student_name << std::endl;
      // Get current time
      time_t now = time(nullptr);
      const tm *ltm = localtime(&now);
      // Write timestamp to file
      saveFileName << 1900 + ltm->tm_year << "-" << 1 + ltm->tm_mon << "-" << ltm->tm_mday << std::endl;
      // Write key to file
      saveFileName << key << std::endl;
      // Write data to file
      saveFileName << data;
      // Close file
      saveFileName.close();
